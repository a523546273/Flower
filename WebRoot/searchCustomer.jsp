...
<link rel="stylesheet" type="text/css"
 ...
 <script type="text/javascript">
	...
	function skipPrev(pageIndex,txtName,txtAddress){
	if(pageIndex==0){
	alert('当前已经是第一页！');
	}else{
	location.href="SearchCustomerServlet?txtName="+txtName.trim()+"&pageIndex="+pageIndex+"&txtAddress="+txtAddress.trim()+"&type=search";
	}
	}
	
	function skipNext(pageIndex,pageCount,txtName,txtAddress){
	...
	function skipPage(pageIndex,txtName,txtAddress){
	location.href="SearchCustomerServlet?txtName="+txtName.trim()+"&pageIndex="+pageIndex+"&txtAddress="+txtAddress.trim()+"&type=search";
	}
	
	function jumpPage(pageCount,txtName,txtAddress){
...
共 <span class="blue">${requestScope.customerCount }</span> 条记录，当前显示第 <span class="blue">${requestScope.pageIndex }</span> 页，共 <span class="blue">${requestScope.pageCount }</span> 页
				...
				<div class="ui_frt">
						<!--    如果是第一页，则只显示下一页、尾页 -->
							...
							onclick="skipPage(1,'<%=request.getAttribute("txtName") %>','<%=request.getAttribute("txtAddress") %>')" />
							<input type="button" value="上一页" class="ui_input_btn01"
							onclick="skipPrev('${requestScope.pageIndex-1 }','<%=request.getAttribute("txtName") %>','<%=request.getAttribute("txtAddress") %>')" />
							<input type="button" value="下一页" class="ui_input_btn01"
							onclick="skipNext('${requestScope.pageIndex+1 }','${requestScope.pageCount }','<%=request.getAttribute("txtName") %>','<%=request.getAttribute("txtAddress") %>')"/>
							<input type="button" value="尾页" class="ui_input_btn01"
							onclick="skipPage('${requestScope.pageCount }','<%=request.getAttribute("txtName") %>','<%=request.getAttribute("txtAddress") %>')"/>
						...
						转到第<input type="text" id="jumpNumTxt" class="ui_input_txt01" name="jumpNumTxt"/>页
							 <input type="button" class="ui_input_btn01" value="跳转" 
								onclick="jumpPage('${requestScope.pageCount }','<%=request.getAttribute("txtName") %>','<%=request.getAttribute("txtAddress") %>')" />
					</div>
				</div>
			</div>
		</div>
